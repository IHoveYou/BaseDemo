...
public class SQUtils extends SQLiteOpenHelper {
    ...
    public static SQUtils getInstance(Context context) {
        if (utils == null)
            utils = new SQUtils(context, "taxiu.db", null, 1);
    ...
    public <T extends Object> T getBean(String id,Class<T> classOfT) {
        T bean = gson.fromJson(get(id),classOfT);
        return bean;
    }
    public <T extends Object> T getBean(String id, Type typeOfT) {
        T bean = gson.fromJson(get(id),typeOfT);
        return bean;
    }
